...
internal sealed class CompanionForm : Form
{
    ...
    private async Task SendFilesAsync(){try{if(files.Count==0)throw new InvalidOperationException("Add at least one file first.");var device=SelectedDevice();SetBusy(true);progress.Value=0;for(var i=0;i<files.Count;i++){var index=i;var path=files[i];var file=new FileInfo(path);status.Text=$"Waiting for approval on {device.Name}: {file.Name}";await AndroidClient.UploadAsync(device,path,ConfirmSecurityCode,(done,total)=>Ui(()=>{var part=total==0?1d:(double)done/total;progress.Value=Math.Clamp((int)(((index+part)/files.Count)*100),0,100);status.Text=$"Encrypted {file.Name} — {FormatBytes(done)} / {FormatBytes(total)}";}));}progress.Value=100;status.Text=$"Complete — {files.Count} encrypted file(s) sent to {device.Name} ✓";MessageBox.Show(this,status.Text,"OptiShare",MessageBoxButtons.OK,MessageBoxIcon.Information);}catch(Exception ex){Fail("OptiShare transfer failed",ex);}finally{SetBusy(false);}}
    private async Task SendClipboardAsync(){try{var text=Clipboard.ContainsText()?Clipboard.GetText():"";if(string.IsNullOrEmpty(text))throw new InvalidOperationException("Windows clipboard has no text to send.");SetBusy(true);var device=SelectedDevice();status.Text=$"Waiting for encrypted clipboard approval on {device.Name}…";await AndroidClient.SendClipboardAsync(device,text,ConfirmSecurityCode);status.Text=$"Encrypted clipboard sent to {device.Name} ✓";}catch(Exception ex){Fail("OptiShare clipboard",ex);}finally{SetBusy(false);}}
    ...
    private bool ConfirmSecurityCode(string code)
    {
        if(InvokeRequired)return (bool)Invoke(new Func<bool>(()=>ConfirmSecurityCode(code)));
        return MessageBox.Show(this,$"Security code: {code}\r\n\r\nConfirm only if Android shows exactly the same six digits.","OptiShare secure connection",MessageBoxButtons.YesNo,MessageBoxIcon.Warning)==DialogResult.Yes;
    }
