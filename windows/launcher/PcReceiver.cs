...
internal sealed class PcReceiver(Func<string,bool> approve, Action<string> notify) : IDisposable
{
    ...
    private bool Allow(string address){if(string.IsNullOrWhiteSpace(address))return false;var now=Environment.TickCount64;while(true){if(!attempts.TryGetValue(address,out var old)){if(attempts.TryAdd(address,(now,1))){TrimAttempts(now);return true;}continue;}var next=now-old.Start>=10000?(Start:now,Count:1):(Start:old.Start,Count:old.Count+1);if(next.Count>12)return false;if(attempts.TryUpdate(address,next,old))return true;}}
