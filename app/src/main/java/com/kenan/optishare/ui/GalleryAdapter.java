...
public final class GalleryAdapter extends RecyclerView.Adapter<GalleryAdapter.Holder> {
    ...
    @NonNull @Override public Holder onCreateViewHolder(@NonNull ViewGroup parent,int viewType){
        ...
        root.addView(preview,new LinearLayout.LayoutParams(-1,dp(parent,118)));
    ...
    @Override public void onDetachedFromRecyclerView(@NonNull RecyclerView recyclerView){releasePlayer();super.onDetachedFromRecyclerView(recyclerView);}
    @Override public int getItemCount(){return items.size();}
    private static void updateBackground(LinearLayout root,boolean checked){GradientDrawable g=new GradientDrawable(GradientDrawable.Orientation.TL_BR,new int[]{checked?0xff174f7d:0xff0d2138,checked?0xff123b62:0xff102c48});g.setCornerRadius(dp(root,18));g.setStroke(dp(root,checked?3:1),checked?0xff50cfff:0xff274b68);root.setBackground(g);root.setScaleX(checked?.97f:1f);root.setScaleY(checked?.97f:1f);}
