...
public final class GalleryAdapter extends RecyclerView.Adapter<GalleryAdapter.Holder> {
    ...
    @NonNull @Override public Holder onCreateViewHolder(@NonNull ViewGroup parent,int viewType){
        ...
        ImageButton play=new ImageButton(parent.getContext());play.setImageResource(android.R.drawable.ic_media_play);play.setColorFilter(Color.WHITE);play.setBackground(pill(0xd92b86d1,28,parent));play.setContentDescription("Preview audio");FrameLayout.LayoutParams playLp=new FrameLayout.LayoutParams(dp(parent,52),dp(parent,52),Gravity.CENTER);if(Build.VERSION.SDK_INT>=21){play.setElevation(dp(parent,14));play.setTranslationZ(dp(parent,5));}play.setOnTouchListener((v,e)->{if(e.getAction()==android.view.MotionEvent.ACTION_DOWN)v.animate().scaleX(.9f).scaleY(.9f).translationY(dp(v,4)).setDuration(70).start();else if(e.getAction()==android.view.MotionEvent.ACTION_UP||e.getAction()==android.view.MotionEvent.ACTION_CANCEL)v.animate().scaleX(1f).scaleY(1f).translationY(0f).setDuration(170).start();return false;});preview.addView(play,playLp);
