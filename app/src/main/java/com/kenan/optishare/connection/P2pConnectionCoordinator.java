 ...
 *
 * Responsibilities:
 * - clear stale Wi-Fi Direct groups before a new session
 * - repeat discovery instead of relying on a single Android callback
 * - expose explicit connection states to the UI
 * - automatically retry BUSY/timeout cases
 * - cancel stale invitations before retrying
 * - keep receiver group ownership deterministic
 *
 * The caller owns runtime-permission checks. This class deliberately contains no UI.
 */
public final class P2pConnectionCoordinator {
    ...
    public P2pConnectionCoordinator(Context context, Listener listener) {
        ...
        this.channel = manager == null ? null : manager.initialize(context, Looper.getMainLooper(),
    ...
    public void startSender() {
        ...
        register();
        emit(State.PREPARING, "Preparing nearby connection…", 0);
        clearStaleState(() -> {
    ...
    public void connect(WifiP2pDevice device) {
        ...
        config.groupOwnerIntent = 0; // receiver owns the group
        ...
        try {
            ...
            emit(State.FAILED, "Nearby permission was blocked by Android.", connectAttempt);
        }
    }

    /** Pairing QR may provide a target address. Discovery continues until that exact peer appears. */
    public void connectWhenFound(String deviceAddress, String displayName) {
        ...
        for (WifiP2pDevice peer : new ArrayList<>(peers)) {
            if (matchesRequested(peer)) {
                connect(peer);
                return;
            }
        }
    ...
    public void stop() {
        ...
        connectingDevice = null;
        try { if (manager != null && channel != null) manager.stopPeerDiscovery(channel, noOp()); }
        catch (Exception ignored) { }
        try { if (manager != null && channel != null) manager.cancelConnect(channel, noOp()); }
        catch (Exception ignored) { }
        unregister();
    ...
    private void createReceiverGroup() {
        ...
        try {
            manager.createGroup(channel, new WifiP2pManager.ActionListener() {
                @Override public void onSuccess() {
                    emit(State.ADVERTISING, "Ready to receive. Keep OptiShare open on the sender.", connectAttempt);
                    requestConnectionInfo();
                }
                ...
                @Override public void onFailure(int reason) {
                    if (reason == WifiP2pManager.BUSY && connectAttempt < MAX_CONNECT_ATTEMPTS) {
                        ...
                        handler.postDelayed(() -> clearStaleState(P2pConnectionCoordinator.this::createReceiverGroup),
            ...
            manager.discoverPeers(channel, new WifiP2pManager.ActionListener() {
                ...
                @Override public void onFailure(int reason) {
                    ...
                    if (reason == WifiP2pManager.BUSY && discoveryFailures <= MAX_DISCOVERY_FAILURES) {
                        emit(State.RETRYING, "Nearby radio is busy. Retrying search automatically…", discoveryFailures);
                    } else if (discoveryFailures > MAX_DISCOVERY_FAILURES) {
                        emit(State.FAILED, "Nearby search did not start. Toggle Wi-Fi and retry.", discoveryFailures);
                    } else {
                        emit(State.SEARCHING, "Searching… " + reasonText(reason), discoveryFailures);
                    }
                }
            });
        } catch (SecurityException e) {
            emit(State.FAILED, "Nearby permission was blocked by Android.", discoveryFailures);
        }
    }
    ...
    private void requestPeers() {
        ...
        try {
            manager.requestPeers(channel, list -> {
                peers.clear();
                if (list != null) peers.addAll(list.getDeviceList());
                Collections.sort(peers, Comparator.comparing(P2pConnectionCoordinator::safeName,
    ...
    private void requestConnectionInfo() {
        try {
            manager.requestConnectionInfo(channel, info -> {
                ...
                handler.removeCallbacks(connectTimeout);
                emit(State.CONNECTED, "Connected securely to nearby device", connectAttempt);
    ...
    private void retryConnect(String message) {
        ...
        if (connectAttempt >= MAX_CONNECT_ATTEMPTS) {
            emit(State.FAILED, message + ". Tap retry or use QR pairing.", connectAttempt);
            connectingDevice = null;
            return;
        }
    ...
    private void clearStaleState(Runnable next) {
        ...
        try {
            ...
            handler.postDelayed(next, 350L);
        }
    }

    private boolean matchesRequested(WifiP2pDevice peer) {
        return requestedAddress != null && peer != null && peer.deviceAddress != null
                && requestedAddress.equalsIgnoreCase(peer.deviceAddress);
    }
    ...
    private static String safeName(WifiP2pDevice device) {
        if (device == null || device.deviceName == null || device.deviceName.trim().isEmpty()) return "Android device";
