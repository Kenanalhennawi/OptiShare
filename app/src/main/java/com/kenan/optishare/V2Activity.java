...
public class V2Activity extends ComponentActivity implements
    ...
    private Button category(int icon,String label,int color,View.OnClickListener listener){Button b=new Button(this);b.setAllCaps(false);b.setText(UiText.get(this,label));b.setTextColor(Color.WHITE);b.setTextSize(14);b.setTypeface(Typeface.DEFAULT_BOLD);b.setGravity(Gravity.CENTER);b.setCompoundDrawablesWithIntrinsicBounds(0,icon,0,0);b.setCompoundDrawablePadding(dp(10));b.setPadding(dp(8),dp(14),dp(8),dp(12));b.setBackground(gradient(lighten(color),darken(color),22));if(Build.VERSION.SDK_INT>=21)b.setElevation(dp(7));applyPressMotion(b);b.setOnClickListener(listener);return b;}
    ...
    private Button bigAction(int icon,String title,String sub,int top,int bottom){Button b=new Button(this);b.setAllCaps(false);b.setText(UiText.get(this,title)+"\n"+UiText.get(this,sub));b.setTextColor(Color.WHITE);b.setTextSize(15);b.setTypeface(Typeface.DEFAULT_BOLD);b.setGravity(Gravity.CENTER);b.setCompoundDrawablesWithIntrinsicBounds(0,icon,0,0);b.setCompoundDrawablePadding(dp(12));b.setPadding(dp(12),dp(18),dp(12),dp(14));b.setBackground(gradient(lighten(top),bottom,26));if(Build.VERSION.SDK_INT>=21)b.setElevation(dp(9));applyPressMotion(b);return b;}
    ...
    private void applyPressMotion(View view){view.setOnTouchListener((v,event)->{if(event.getAction()==android.view.MotionEvent.ACTION_DOWN)v.animate().scaleX(.96f).scaleY(.96f).setDuration(90).start();else if(event.getAction()==android.view.MotionEvent.ACTION_UP||event.getAction()==android.view.MotionEvent.ACTION_CANCEL)v.animate().scaleX(1f).scaleY(1f).setDuration(140).start();return false;});}
