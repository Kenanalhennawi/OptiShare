...
public final class V3TransferActivity extends ComponentActivity implements P2pConnectionCoordinator.Listener {
        ...
        @Override public void onReceive(Context context, Intent intent) {
            ...
            switch (event) {
                ...
                case "file_done":
                    ...
                    break;
                case "completed":
                    setStatus("Transfer complete ✓", MODE_RECEIVE.equals(mode)
                            ? "Files are ready in your Received Library"
                            : "All selected files were sent", 2);
    ...
    private void renderSelection() {
        ...
        Button photos = category("▣", "Photos", Color.rgb(29, 151, 255));
        photos.setOnClickListener(v -> openMedia("image"));
        Button videos = category("▶", "Videos", Color.rgb(126, 91, 246));
        videos.setOnClickListener(v -> openMedia("video"));
        grid1.addView(photos, new LinearLayout.LayoutParams(0, dp(118), 1));
        LinearLayout.LayoutParams vp = new LinearLayout.LayoutParams(0, dp(118), 1);
        vp.setMargins(dp(10), 0, 0, 0);
        grid1.addView(videos, vp);
        contentArea.addView(grid1);

        LinearLayout grid2 = new LinearLayout(this);
        grid2.setOrientation(LinearLayout.HORIZONTAL);
        Button music = category("♫", "Music", Color.rgb(35, 190, 146));
        music.setOnClickListener(v -> openMedia("audio"));
        Button apps = category("A", "Apps", Color.rgb(236, 108, 76));
        apps.setOnClickListener(v -> appPicker.launch(new Intent(this, AppPickerActivity.class)));
        grid2.addView(music, new LinearLayout.LayoutParams(0, dp(118), 1));
        LinearLayout.LayoutParams ap = new LinearLayout.LayoutParams(0, dp(118), 1);
        ap.setMargins(dp(10), 0, 0, 0);
        grid2.addView(apps, ap);
        LinearLayout.LayoutParams g2 = new LinearLayout.LayoutParams(-1, -2);
        g2.setMargins(0, dp(10), 0, 0);
        contentArea.addView(grid2, g2);

        Button files = category("▤", "Documents & files", Color.rgb(245, 158, 54));
        files.setOnClickListener(v -> openExternal("*/*"));
        LinearLayout.LayoutParams fp = new LinearLayout.LayoutParams(-1, dp(98));
        fp.setMargins(0, dp(10), 0, 0);
        contentArea.addView(files, fp);

        LinearLayout batch = card();
        TextView batchTitle = text(selected.size() + " item" + (selected.size() == 1 ? "" : "s") + " • " + formatBytes(selectedBytes()), 16, Color.WHITE, true);
        batch.addView(batchTitle);
        if (selected.isEmpty()) {
            batch.addView(text("Nothing selected yet", 12, Color.rgb(137, 166, 190), false));
        } else {
            for (int i = 0; i < Math.min(6, selected.size()); i++) {
                batch.addView(text("✓ " + displayName(selected.get(i)), 12, Color.rgb(219, 232, 242), false));
            }
            if (selected.size() > 6) {
                batch.addView(text("+ " + (selected.size() - 6) + " more", 12, Color.rgb(91, 203, 255), true));
            }
            Button clear = secondary("Clear selection");
            clear.setOnClickListener(v -> {
                selected.clear();
                renderSelection();
            });
            LinearLayout.LayoutParams cl = new LinearLayout.LayoutParams(-1, dp(46));
            cl.setMargins(0, dp(10), 0, 0);
            batch.addView(clear, cl);
        }
        LinearLayout.LayoutParams bp = new LinearLayout.LayoutParams(-1, -2);
        bp.setMargins(0, dp(14), 0, 0);
        contentArea.addView(batch, bp);
        ...
        continueButton.setAlpha(selected.isEmpty() ? .45f : 1f);
        continueButton.setOnClickListener(v -> startDiscoveryFlow());
        LinearLayout.LayoutParams cont = new LinearLayout.LayoutParams(-1, dp(60));
        cont.setMargins(0, dp(14), 0, 0);
        contentArea.addView(continueButton, cont);
    }
    ...
    private void startDiscoveryFlow() {
        ...
        LinearLayout visual = card();
        TextView radar = text("◎", 74, Color.rgb(89, 207, 255), true);
        radar.setGravity(Gravity.CENTER);
        visual.addView(radar);
        TextView hint = text("Looking for OptiShare receivers…", 15, Color.WHITE, true);
        hint.setGravity(Gravity.CENTER);
        visual.addView(hint);
        contentArea.addView(visual);

        LinearLayout actions = new LinearLayout(this);
        actions.setOrientation(LinearLayout.HORIZONTAL);
        Button qr = secondary("Scan receiver QR");
        qr.setOnClickListener(v -> scanQr());
        actions.addView(qr, new LinearLayout.LayoutParams(0, dp(50), 1));
        Button retry = secondary("Restart search");
        retry.setOnClickListener(v -> coordinator.startSender());
        ...
        contentArea.addView(actions, actionsLp);
        ...
        pl.setMargins(0, dp(12), 0, 0);
        contentArea.addView(peerList, pl);
        renderPeers();
    ...
    private void startReceiveFlow() {
        ...
        TextView mark = text("◉", 78, Color.rgb(61, 220, 151), true);
        mark.setGravity(Gravity.CENTER);
        receiveCard.addView(mark);
        TextView title = text("Ready for nearby transfers", 19, Color.WHITE, true);
        title.setGravity(Gravity.CENTER);
        receiveCard.addView(title);
        TextView device = text(identity.name(), 13, Color.rgb(91, 204, 255), true);
        device.setGravity(Gravity.CENTER);
        device.setTag("device_label");
        receiveCard.addView(device);
        receiverQr = new ImageView(this);
        receiverQr.setAdjustViewBounds(true);
        receiverQr.setContentDescription("OptiShare receiver pairing QR code");
        receiverQr.setPadding(dp(24), dp(18), dp(24), dp(10));
        receiveCard.addView(receiverQr, new LinearLayout.LayoutParams(-1, dp(300)));
        contentArea.addView(receiveCard);
        ...
        explain.setPadding(dp(10), dp(12), dp(10), 0);
        contentArea.addView(explain);
    ...
    private void renderTransfer() {
        ...
        progressTitle = text(MODE_RECEIVE.equals(mode) ? "Receiving" : "Sending", 22, Color.WHITE, true);
        transferCard.addView(progressTitle);
        progressDetail = text("Preparing encrypted session…", 13, Color.rgb(150, 182, 205), false);
        transferCard.addView(progressDetail);
        progress = new ProgressBar(this, null, android.R.attr.progressBarStyleHorizontal);
        progress.setMax(100);
        LinearLayout.LayoutParams pp = new LinearLayout.LayoutParams(-1, dp(9));
        pp.setMargins(0, dp(14), 0, 0);
        transferCard.addView(progress, pp);
        contentArea.addView(transferCard);

        Button cancel = secondary("Cancel transfer");
        cancel.setOnClickListener(v -> new AlertDialog.Builder(this)
                .setTitle("Cancel transfer?")
                .setMessage("This will clear this pending transfer session. Files already completed remain valid.")
                .setPositiveButton("Cancel transfer", (d, w) -> cancelTransfer())
        ...
        contentArea.addView(cancel, cp);
    }

    private void cancelTransfer() {
        Intent stop = new Intent(this, TransferService.class).setAction(TransferService.ACTION_STOP);
        startService(stop);
    ...
    private void updateTransfer(String title, String detail, int p) {
        runOnUiThread(() -> {
            if (progressTitle != null) progressTitle.setText(title);
            if (progressDetail != null) progressDetail.setText(detail == null ? "" : detail);
            if (progress != null && p >= 0) progress.setProgress(p);
        });
    }

    private void addReceivedButton() {
        runOnUiThread(() -> {
            Button open = primary("Open received files");
            open.setOnClickListener(v -> startActivity(new Intent(this, ReceivedActivity.class)));
            LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(-1, dp(58));
            lp.setMargins(0, dp(12), 0, 0);
            contentArea.addView(open, lp);
        });
    }

    @Override public void onState(P2pConnectionCoordinator.State state, String message, int attempt) {
        int kind = (state == P2pConnectionCoordinator.State.CONNECTED || state == P2pConnectionCoordinator.State.ADVERTISING)
                ? 2 : (state == P2pConnectionCoordinator.State.FAILED ? 3 : 1);
        String title;
        switch (state) {
            case PREPARING: title = "Preparing"; break;
            case ADVERTISING: title = "Ready to receive"; break;
            case SEARCHING: title = "Searching"; break;
            case FOUND: title = "Devices found"; break;
            case CONNECTING: title = "Connecting"; break;
            case NEGOTIATING: title = "Securing connection"; break;
            case CONNECTED: title = "Connected ✓"; break;
            case RETRYING: title = "Retrying automatically"; break;
            case FAILED: title = "Connection needs attention"; break;
            default: title = "OptiShare"; break;
        }
        setStatus(title, message, kind);
    }

    @Override public void onPeers(List<WifiP2pDevice> list) {
        peers.clear();
    ...
    @Override public void onConnected(WifiP2pInfo info, String peerName) {
        ...
        if (MODE_RECEIVE.equals(mode) && info.isGroupOwner) {
            Intent intent = new Intent(this, TransferService.class)
                    .setAction(TransferService.ACTION_START_RECEIVER)
                    .putExtra(TransferService.EXTRA_PEER_NAME, connectedPeer);
            ...
            Intent intent = new Intent(this, TransferService.class)
                    ...
                    .putExtra(TransferService.EXTRA_HOST, info.groupOwnerAddress.getHostAddress())
                    .putExtra(TransferService.EXTRA_PEER_NAME, connectedPeer)
                    .putStringArrayListExtra(TransferService.EXTRA_URIS, raw);
            ContextCompat.startForegroundService(this, intent);
        } else {
            setStatus("Connection role mismatch", "Retry the nearby connection. The receiving phone must own the direct session.", 3);
        }
    }
    ...
    @Override public void onThisDevice(WifiP2pDevice device) {
        ...
        runOnUiThread(() -> {
            try {
                if (receiverQr != null) {
                    receiverQr.setImageBitmap(makeQr("OPTISHARE3|" + device.deviceAddress + "|" + identity.name(), 720));
                }
                TextView label = findViewByTag("device_label");
                if (label != null) label.setText(identity.name() + " • " + safeName(device));
    ...
    private void renderPeers() {
        runOnUiThread(() -> {
            ...
            if (peers.isEmpty()) {
                ...
                empty.addView(text("Searching automatically…", 14, Color.WHITE, true));
                empty.addView(text("No receiver yet. Keep Receive open on the other phone.", 12, Color.rgb(141, 171, 194), false));
            ...
            for (WifiP2pDevice device : peers) {
                ...
                LinearLayout row = new LinearLayout(this);
                row.setGravity(Gravity.CENTER_VERTICAL);
                TextView avatar = text(firstLetter(safeName(device)), 18, Color.WHITE, true);
                avatar.setGravity(Gravity.CENTER);
                avatar.setBackground(round(Color.rgb(38, 139, 211), 18));
                row.addView(avatar, new LinearLayout.LayoutParams(dp(48), dp(48)));
                LinearLayout labels = new LinearLayout(this);
                labels.setOrientation(LinearLayout.VERTICAL);
                labels.setPadding(dp(12), 0, 0, 0);
                labels.addView(text(safeName(device), 15, Color.WHITE, true));
                labels.addView(text(peerStatus(device.status), 11, Color.rgb(143, 177, 201), false));
                row.addView(labels, new LinearLayout.LayoutParams(0, -2, 1));
                Button connect = secondary("Connect");
                connect.setOnClickListener(v -> coordinator.connect(device));
                row.addView(connect, new LinearLayout.LayoutParams(dp(105), dp(46)));
                deviceCard.addView(row);
                LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(-1, -2);
                lp.setMargins(0, 0, 0, dp(8));
                peerList.addView(deviceCard, lp);
            }
        });
    }
    ...
    private void openMedia(String type) {
        ...
        intent.putExtra(MediaPickerActivity.EXTRA_TYPE, type);
        ArrayList<String> initial = new ArrayList<>();
        for (Uri uri : selected) initial.add(uri.toString());
        intent.putStringArrayListExtra(MediaPickerActivity.EXTRA_INITIAL, initial);
    ...
    private boolean ensureNearbyReady() {
        if (!coordinator.available()) {
            showMessage("Nearby sharing unavailable", "This device does not expose Android Wi-Fi Direct.");
            return false;
        }
    ...
    private void requestNearbyPermission() {
        if (Build.VERSION.SDK_INT >= 33) {
            ...
            requestPermissions(new String[]{Manifest.permission.ACCESS_COARSE_LOCATION, Manifest.permission.ACCESS_FINE_LOCATION}, REQ_NEARBY);
        }
    }

    private boolean ensureLegacyWrite() {
    ...
    @Override public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == REQ_NEARBY && hasNearbyPermission()) {
            if (MODE_RECEIVE.equals(mode)) startReceiveFlow(); else startDiscoveryFlow();
        } else if (requestCode == REQ_WRITE && ensureLegacyWrite() && MODE_RECEIVE.equals(mode)) {
            startReceiveFlow();
        }
    }

    private void setStatus(String title, String detail, int kind) {
        runOnUiThread(() -> {
            if (statusTitle == null) return;
            statusTitle.setText(title);
            statusDetail.setText(detail == null ? "" : detail);
            int color = kind == 2 ? Color.rgb(63, 220, 147)
                    : kind == 3 ? Color.rgb(255, 91, 101)
                    : kind == 1 ? Color.rgb(255, 193, 70)
                    : Color.rgb(116, 145, 168);
            statusDot.setBackground(round(color, 20));
        });
    }

    private void mergeRawUris(ArrayList<String> raw) {
        if (raw == null) return;
        for (String value : raw) {
            try { addUnique(Uri.parse(value)); } catch (Exception ignored) { }
        }
    }

    private void addUnique(Uri uri) {
        ...
        for (Uri uri : selected) {
            ...
            try {
                ...
                if (cursor != null && cursor.moveToFirst()) {
                    ...
                    if (index >= 0 && !cursor.isNull(index)) total += Math.max(0, cursor.getLong(index));
                }
            } catch (Exception ignored) {
            } finally {
                if (cursor != null) cursor.close();
    ...
    private static String peerStatus(int status) {
        if (status == WifiP2pDevice.CONNECTED) return "Connected";
        if (status == WifiP2pDevice.INVITED) return "Invitation sent";
        if (status == WifiP2pDevice.AVAILABLE) return "Available";
        if (status == WifiP2pDevice.UNAVAILABLE) return "Busy";
        return "Nearby";
    }
    ...
    private <T extends View> T findViewByTag(String tag) {
        View view = getWindow().getDecorView().findViewWithTag(tag);
        return (T) view;
    }
    ...
    private LinearLayout card() {
        ...
        GradientDrawable background = round(Color.rgb(13, 32, 52), 20);
        background.setStroke(dp(1), Color.rgb(31, 61, 87));
        layout.setBackground(background);
        return layout;
    }

    private Button category(String icon, String label, int color) {
    ...
    private Button primary(String label) {
        ...
        button.setTypeface(Typeface.DEFAULT_BOLD);
        GradientDrawable background = new GradientDrawable(GradientDrawable.Orientation.TL_BR,
                new int[]{Color.rgb(30, 156, 255), Color.rgb(78, 75, 230)});
    ...
    private Button secondary(String label) {
        ...
        button.setBackground(round(Color.rgb(23, 49, 72), 16));
        return button;
    }
    ...
    private int darken(int color) {
        return Color.rgb((int) (Color.red(color) * .62),
                (int) (Color.green(color) * .62),
                (int) (Color.blue(color) * .62));
    }
    ...
    @Override protected void onResume() {
        super.onResume();
        IntentFilter filter = new IntentFilter(TransferService.ACTION_EVENT);
